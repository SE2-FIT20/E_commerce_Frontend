...
const DeliveryAllOrders = () => {
  ...
  const handleChangeOrderType = (e) => {
    ...
    history.push(
      `/delivery-partner/${handleDisplayOrderType(
        e.currentTarget.id
      )}?page=${currentPage}`
    );
  };
  ...
  const fetchOrders = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${BACKEND_URL}/api/delivery-partner/orders?elementsPerPage=${orderPerPage}&page=${
          currentPage - 1
  ...
  const fetchOrderTypeCount = async () => {
    try {
      const { data } = await axios.get(
        `${BACKEND_URL}/api/delivery-partner/orders-count`,
        config
      );
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th style={{ flex: "2" }}>Destination Address</th>
